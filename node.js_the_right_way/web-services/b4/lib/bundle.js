...
module.exports = function(config, app) {
  ...
  app.del('/api/bundle/:id/book/:pgid', function(req, res) {
  	console.log('in delete book from bundle handler');
    Q.async(function* (){
  ...
  /**
   * remove a bundle
   * curl -X DELETE http://localhost:3000/api/bundle/<id>/rev/<rev>
   */
  app.del('/api/bundle/:id/rev/:rev', function(req, res) {
  	console.log('in delete bundle handler');
    ...
    Q.async(function* (){
      ...
      // delete the bundle from CouchDB
      request.del({ url: config.b4db + bundle._id + "?rev=" + bundle._rev }).pipe(res);
